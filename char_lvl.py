...
Char Level Calculator update notes:
	-Added long term tracking/saving of levels. (you can now save the results instead of having to load data everytime)
	-Added multiple magics/jobs/skills tracking.
	-Easily edit list of magics/jobs.
	-Saving now also updates the UI to display correct magic levels.
    -Added multiple character support. (you can now switch between characters)
    -Added session XP tracking, for when you want to keep track of several XP gains. Session XP is saved and loaded with the character data, unless you reset it.
...
Todos:
    -Introduce Magics/Jobs linking, where leveling up one magic/job will affect another. (i.e. gaining XP in magic smithing will also give XP in blacksmithing)
    -Add Passives support, where you can add passive skills that give bonuses to other magics/jobs. (i.e. Great Student passive gives 1.5x XP to all magics/jobs)
    -Have to figure out if program fails for very large numbers. (i.e. 1 000 000 000 XP)
...
def update_last_loaded_flag(current_character_name):
    """
    Update the last_loaded flag for all character data files except the current character.
    """
        ...
        if char.lower() != current_character_name.lower():
            ...
            if os.path.exists(file_path):
                with open(file_path, mode="r") as file:
                    ...
                    try:
                        char_data[0][2] = False  # Set last_loaded flag to False
                        with open(file_path, mode="w", newline="") as file:
                            writer = csv.writer(file)
                            writer.writerows(char_data)
                    except IndexError:
                        char_data[0].append(False)  # Add last_loaded flag
                with open(file_path, mode="w", newline="") as file:
                    writer = csv.writer(file)
                    writer.writerows(char_data)
...
def save_calculated_xp():
    ...
    if selected_magic_index is not None:
        ...
        update_magic_data(
            selected_magic_index, new_level, new_xp, next_xp, skip_session_xp
        )
...
def load_character_data(path=None, character_name=None, skip_session_xp=False):
    ...
    if os.path.exists(file_path):
        with open(file_path, mode="r") as file:
            ...
            # Update last_loaded to True for the loaded character
            char_data[0][2] = True
            save_character_data(char_data[2:], name=CHARACTER_NAME, last_loaded=True)
            update_last_loaded_flag(
                CHARACTER_NAME
            )  # Update last_loaded flag for other characters
        ...
        save_character_data(DEFAULT_CHAR_MAGICS, name=character_name)
        return DEFAULT_CHAR_MAGICS
...
def save_character_data(data, name=CHARACTER_NAME, last_loaded=False):
    ...
    with open(f"character_data_{name.lower()}.csv", mode="w", newline="") as file:
        ...
        )  # Write character name and last_loaded flag
        writer.writerow(
...
def create_character_data():
    ...
    if path:
        with open(path, mode="w", newline="") as file:
            ...
            writer.writerows(DEFAULT_CHAR_MAGICS)  # Write default character data
    return path
...
def create_magic_buttons():
    ...
    # Load character data based on last_loaded flag
    ...
    for char in char_list:
        ...
        if os.path.exists(file_path):
            with open(file_path, mode="r") as file:
                reader = csv.reader(file)
                char_data = list(reader)
                if char_data[0][2] == "True":  # Check if last_loaded flag is True
                    last_loaded_character = char
                    break

    if last_loaded_character:
        char_data = load_character_data(character_name=last_loaded_character)
    else:
        char_data = load_character_data(character_name="Default")

    CHARACTER_NAME = (
        last_loaded_character.capitalize() if last_loaded_character else "Default"
    )
...
# Ensure to call the update_last_loaded_flag function whenever a new character is loaded
def select_character(character_name, window):
    ...
    load_and_close(character_name, window)
...
def update_magic_data(index, new_level, new_xp, next_xp, skip_session_xp):
    ...
    char_data = load_character_data(
        character_name=CHARACTER_NAME, skip_session_xp=skip_session_xp
    )
